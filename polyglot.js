...
/*
Polyglot is a Tampermonkey userscript for Codewars with small, opt-in UI improvements.
...
function applyCodeMirrorSettings() {
    ...
    const configured = {
        tabSize:           glotGetOption("codeMirrorTabSize"          ),
    ...
    const isSet = prop => prop && prop !== "Default";

    if(isSet(configured.indentWithTabs   )) optsToSet.indentWithTabs    = (configured.indentWithTabs    === "Yes");
    if(isSet(configured.autoCloseBrackets)) optsToSet.autoCloseBrackets = (configured.autoCloseBrackets === "Yes");
    if(isSet(configured.smartIndent      )) optsToSet.smartIndent       = (configured.smartIndent       === "Yes");

    if(isSet(configured.tabSize) && typeof(configured.tabSize) === 'number') {
        optsToSet.tabSize = Number(configured.tabSize);
    }

    if(Object.getOwnPropertyNames(optsToSet).length)
        unsafeWindow.localStorage.setItem("editorOptions", JSON.stringify(optsToSet));
    else
        unsafeWindow.localStorage.removeItem("editorOptions");
}
